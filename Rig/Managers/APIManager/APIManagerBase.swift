...
class APIManagerBase: NSObject
{
    ...
    override init()
    {
        ...
        configuration.timeoutIntervalForRequest = TimeInterval(Constants.RIGRequestTimeOut)
        configuration.timeoutIntervalForResource = TimeInterval(Constants.RIGResponseTimeOut)

//        let serverTrustPolicies: [String: ServerTrustEvaluating] = [
//            "ec2-52-4-186-178.compute-1.amazonaws.com": DisabledTrustEvaluator()
//            ]
        let serverTrustPolicies: [String: ServerTrustEvaluating] = [
            "serverurl.com": DisabledTrustEvaluator()
            ]
    ...
    //MARK:- Get Authorization Headers
     ...
     {
        let accessToken : String = RIG.DataManager.headers?.accessToken ?? ""
        let httpHeaders = HTTPHeaders.init(["Content-Type":Constants.ContentType,"api-key":Constants.APIKey,"Authorization":"Bearer \(accessToken)"])
        return httpHeaders


       // return ["uid":uid,"client": client,"access-token":accessToken]
     }
    func getUserMultiPartHeader ()  -> HTTPHeaders
    {
       let accessToken : String = RIG.DataManager.headers?.accessToken ?? ""
       let httpHeaders = HTTPHeaders.init(["Content-Type":Constants.ContentTypeMultiPart,"api-key":Constants.APIKey,"Authorization":"Bearer \(accessToken)"])
       return httpHeaders
    ...
    func getUserHeader ()  -> Dictionary<String,String>
    {
        ...
        return ["Content-Type":Constants.ContentType,"api-key":Constants.APIKey,"Authorization":"Bearer \(accessToken)"]
    }
    ...
    func getTokenHeader()  -> HTTPHeaders
    {
        // Live  admin token
        let accessToken : String =  RIG.DataManager.headers?.accessToken ?? ""
        let httpHeaders = HTTPHeaders.init(["Content-Type":Constants.ContentType,
    ...
    {
        let httpHeaders = HTTPHeaders.init( ["Content-Type":Constants.ContentType, "api-key": Constants.APIKey])
        return httpHeaders
    }
...
//MARK: - Post Requests
    func postRequestWith(route: URL,parameters: [String: Any],
        ...
        var urlRequest = URLRequest(
            ...
            timeoutInterval: TimeInterval(Constants.RIGRequestTimeOut)) //20.0 * 1000
        ...
        let task = urlsession.dataTask(with: urlRequest){ (data, response, error) -> Void in
            ...
            guard error == nil else {
               // self.showErrorMessage(error: error!)
                let error = RIG.ValidationManager.alamofireErrorValidation(errorMessage:  error?.asAFError?.errorDescription)
    ...
    func postRequestDefaultTokenWith(route: URL,parameters: Parameters,
        ...
        alamoFireManager.request(route, method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: getDefaultTokenHeader()).responseJSON
            {
            response in
            ...
            guard response.error == nil else{
                ...
                let error = RIG.ValidationManager.alamofireErrorValidation(errorMessage: response.error?.errorDescription)
                failure(error as NSError)
                return;
            }
            if let value = response.value {
                ...
                if let jsonResponse = response.data
                {
                    ...
                    if !(RIG.DataManager.isUserLoggedIn ?? false)
                    {
                        ...
                        header.accessToken = response.response?.headers["access-token"]
                        RIG.DataManager.headers = header
                    }
                    if !(RIG.DataManager.isCompanyLoggedIn ?? false)
                    {
                        var header : RIGHeaders = RIGHeaders()
                        header.accessToken = response.response?.headers["access-token"]
                        RIG.DataManager.headers = header
...
//                    RIG.DataManager.headers?.accessToken = response.response?.headers["access-token"]
//                    RIG.DataManager.headers?.client = response.response?.headers["client"]
//                    RIG.DataManager.headers?.uid = response.response?.headers["uid"]
    ...
    func postRequestWithParamsAndUserToken(route: URL,parameters: Parameters,
                                   success:@escaping ResultAPISuccessClosure,
                                failure:@escaping DefaultAPIFailureClosure){

        alamoFireManager.request(route, method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: getUserHeader()).responseJSON
            {
            response in
    ...
    func postRequestWithUserTokenwithParams(route: URL,parameters: Parameters,
                                  success:@escaping ResultAPISuccessClosure,
                                  failure:@escaping DefaultAPIFailureClosure)
    {

        alamoFireManager.request(route, method: .post,parameters:parameters, encoding:URLEncoding.default, headers: getUserHeader()).responseJSON{
            response in
                                  ...
                                  failure:@escaping DefaultAPIFailureClosure)
    {
        ...
        alamoFireManager.request(route, method: .post, encoding: JSONEncoding.default, headers: getUserHeader()).responseJSON{
            response in
    ...
    // get request with concated prams with header
    func getRequestWithUserTokenwithParams(route: URL,parameters: Parameters,
        ...
        alamoFireManager.request(urlWithParams, method: .get, encoding: JSONEncoding.default, headers: getUserHeader()).responseJSON{
            response in
    ...
    func getRequestWithUserTokenNoParams(route: URL,
                                    success:@escaping ResultAPISuccessClosure,
                                    failure:@escaping DefaultAPIFailureClosure)
    {
                                                ...
                                                if let value = response.value as? [String: AnyObject]
                                                {
                                                    if value["data"] != nil
                                                    {
                                                         if let data = value["data"]
                                                         {
                                                            ...
                                                            {
                                                             //RIG.DataManager.isMembershipDataString = false
                                                            }
                                                            else
                                                            {
                                                            // RIG.DataManager.isMembershipDataString = true
                                                             }
                                                            }
                                                         }
                                                        else
    ...
    func deleteRequestWithUserToken(route: URL,parameters: Parameters,
                                  success:@escaping ResultAPISuccessClosure,
                                  failure:@escaping DefaultAPIFailureClosure)
    {

        alamoFireManager.request(route, method: .delete, encoding: JSONEncoding.default, headers: getUserHeader()).responseJSON{
            response in
    ...
    func rawGetRequest(route: URL,parameters: Parameters,
        ...
        let request = NSMutableURLRequest(url: route,
                                          cachePolicy: .reloadIgnoringLocalAndRemoteCacheData,
                                          timeoutInterval: TimeInterval(Constants.RIGRequestTimeOut)) //1000000000.0
    ...
    func rawPostRequest(_ isRequestType : Bool = false , route: URL,parameters: Parameters,
        ...
        request.allHTTPHeaderFields = getUserHeader()
        request.timeoutInterval =  TimeInterval(Constants.RIGRequestTimeOut) //600000
        request.cachePolicy = .reloadIgnoringLocalAndRemoteCacheData
        UIApplication.shared.isNetworkActivityIndicatorVisible = true
    ...
    func putRequestWith(route: URL,parameters: Parameters, isAdminRequest : Bool = false,
        ...
        alamoFireManager.request(route, method: .put, parameters: parameters, encoding: JSONEncoding.default, headers: httpHeader).responseJSON {
            response in
    ...
    func putRequestWithoutHeaders(route: URL,parameters: Parameters, isAdminRequest : Bool = false,
                        success:@escaping ResultAPISuccessClosure,
                        failure:@escaping DefaultAPIFailureClosure) {

        alamoFireManager.request(route, method: .put, parameters: parameters, encoding: JSONEncoding.default, headers: getDefaultTokenHeader()).responseJSON {
            response in
    ...
    func postRequestWithMultipartImage(route: URL,parameters: Parameters,
                                  success:@escaping ResultAPISuccessClosure,
                                  failure:@escaping DefaultAPIFailureClosure,
                                  errorPopup: Bool){
...
//pdf uploading mathod
    func postRequestMultiPartPDFUpload(route: URL,DocData :Data?,fileName:String?,parameters: Parameters,
                                       success:@escaping ResultAPISuccessClosure,
                                       failure:@escaping DefaultAPIFailureClosure){

        let URLSTR = try! URLRequest(url: route.absoluteString, method: HTTPMethod.post, headers: getUserMultiPartHeader())
        AF.upload(multipartFormData: { multipartFormData in
            if let data = DocData{
    ...
    func postRequestWithMultipartAudio(route: URL,parameters: Parameters,
                                  success:@escaping ResultAPISuccessClosure,
                                  failure:@escaping DefaultAPIFailureClosure,
                                  errorPopup: Bool){
